  ...
  return (
    <div className="space-y-4">
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 card-shadow p-4 space-y-3">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="relative">
            ...
            <select className="h-10 pl-9 pr-4 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm appearance-none" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as OrderStatus | 'Todos')}>
        ...
        <div className="flex flex-col sm:flex-row gap-2 items-center">
          ...
          <input type="date" className="h-9 px-3 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} />
          <span className="text-xs text-slate-400">até</span>
          <input type="date" className="h-9 px-3 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm" value={dateTo} onChange={(e) => setDateTo(e.target.value)} />
      ...
      <div className="md:hidden space-y-3">
        {filteredOrders.map((order) => (
          <div key={order.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 card-shadow p-4 space-y-2" onClick={() => setSelectedOrder(order)}>
      ...
      <Modal isOpen={!!selectedOrder} onClose={() => setSelectedOrder(null)} title={`Pedido #${selectedOrder?.id.substring(0, 8)}`}>
        {selectedOrder && (
          <div className="space-y-4">
            ...
            {selectedOrder.justificativa && (<div className="text-sm"><span className="block text-slate-500 mb-1">Justificativa</span><p className="bg-slate-50 dark:bg-slate-700 p-3 rounded-md">{selectedOrder.justificativa}</p></div>)}
            {selectedOrder.observacao && (<div className="text-sm"><span className="block text-slate-500 mb-1">Observação do Gestor</span><p className="bg-slate-50 dark:bg-slate-700 p-3 rounded-md">{selectedOrder.observacao}</p></div>)}
            <div>
